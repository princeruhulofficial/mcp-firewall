...
mcp-firewall should evolve from a single MCP security tool into a **Zero Trust AI Execution Layer** backed by a **Universal Context Protocol (UCP)** direction: a default control plane and runtime guard between AI agents, MCP servers, tools, APIs, files, databases, and production systems.
...
MCP is the first tool-calling adapter. UCP is the broader context and action standard. The Zero Trust AI Execution Layer is the enforcement system that uses UCP context to apply identity, permission, risk, approval, audit, and capability controls.

See [Universal Context Protocol](universal-context-protocol.md) for the portable context envelope and standard action lifecycle.
...
- Add UCP envelopes for non-MCP actions.
